...
Auth <- function(username,pw,button){
  ...
  if(length(validUsers[[1]])==0){
    login = F 
  } else if(validUsers$password==pw) {
...
Update_profile <- function(username,name,surname,password,team){
  user_base <- readRDS("Users/Users.Rds")
  validUsers <- dplyr::filter(user_base,user==username)
...
newrow <- data.frame("user"=username,"password" = password,"permissions"=permission,"name"=name,"surname"=surname,"team"=team)
print(newrow)
...
newuser_profile <- function(username,name,surname,password,team){
  user_base <- readRDS("Users/Users.Rds")
